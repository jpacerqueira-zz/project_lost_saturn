...
if [[ $DOCKERLS != "jupyter_container" ]]; then
   docker run  -itd -p 9003:9003 --cap-add=NET_ADMIN --net=container:vpn  --name jupyter_container --mount type=bind,source="$(pwd)"/notebooks,target=${DEST_FOLDER} jpacerqueira83/jupyter_datascience:latest-stable-version-2.4.5
